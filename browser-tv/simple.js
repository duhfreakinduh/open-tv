...
const list=document.querySelector('#list'),groups=document.querySelector('#groups'),search=document.querySelector('#search'),status=document.querySelector('#status'),nameEl=document.querySelector('#name'),details=document.querySelector('#details'),playBtn=document.querySelector('#play'),video=document.querySelector('#video');
let shown=[...ITEMS],idx=0,active='All';
function esc(s=''){return s.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function renderGroups(){groups.innerHTML=GROUPS.map(g=>`<button class="${g===active?'on':''}" data-g="${g}">${g}</button>`).join('');groups.querySelectorAll('button').forEach(b=>b.onclick=()=>{active=b.dataset.g;renderGroups();filter()})}
function filter(){const q=search.value.toLowerCase().trim();shown=ITEMS.filter(x=>(active==='All'||x.group===active)&&(!q||(x.name+' '+x.description+' '+x.group).toLowerCase().includes(q)));list.innerHTML=shown.length?shown.map((x,i)=>`<article class="channel" data-i="${i}">${x.logo?`<img src="${esc(x.logo)}" alt="" onerror="this.outerHTML='<div class=&quot;fallback&quot;>${esc(x.name.slice(0,2).toUpperCase())}</div>'">`:`<div class="fallback">${esc(x.name.slice(0,2).toUpperCase())}</div>`}<div><b>${esc(x.name)}</b><p>${esc(x.description)}</p></div><span class="tag">${esc(x.group)}</span></article>`).join(''):'<div class="empty">No services found</div>';list.querySelectorAll('.channel').forEach(r=>r.onclick=()=>select(+r.dataset.i));status.textContent=`${shown.length} services`;if(shown.length)select(Math.min(idx,shown.length-1),false)}
function select(i,open=false){if(!shown.length)return;idx=(i+shown.length)%shown.length;const c=shown[idx];nameEl.textContent=c.name;details.textContent=c.description;status.textContent=`Ready: ${c.name}`;video.removeAttribute('src');video.poster=c.logo||'';video.load();playBtn.textContent='▶';if(open)window.open(c.url,'_blank','noopener,noreferrer')}
function launch(){if(!shown.length)return;const c=shown[idx];status.textContent=`Opening ${c.name}…`;window.open(c.url,'_blank','noopener,noreferrer')}
document.querySelector('#prev').onclick=()=>select(idx-1);document.querySelector('#next').onclick=()=>select(idx+1);playBtn.onclick=launch;search.oninput=filter;renderGroups();filter();
